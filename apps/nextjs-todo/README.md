...
Both are landing pages for an **emailed link**, so the link needs a URL to arrive at. That is
what the `resetPasswordUrl` and `verifyEmailUrl` props take.

AuthOwl owns all the logic and UI. The components are one line each — the rest of those files
is the shared `<AuthShell>` wrapper, for looks:
...
**You do not have to host them.** AuthOwl's hosted account portal serves `/reset-password`
and `/verify-email` too, mounting those exact components with your project's branding. This
example mounts them locally for two practical reasons: the portal is opt-in and needs an
accounts domain configured, so a fork would not run without setup; and its origin is not in
the SDK's public config, so it cannot be discovered — you would paste another URL into env.

With the portal enabled, delete both directories and point the props at it instead:
...
What you cannot do is drop the props. Omit `resetPasswordUrl` and the **"Forgot password?"
link disappears from the sign-in form entirely** — it is gated on the prop — so password
reset quietly stops being reachable. Omit `verifyEmailUrl` and sign-up sends no
`callbackURL`, so the confirmation link never returns the user to your app.
